...
const DoctorDashboard = async  () => {
  ...
  return(
        <Tabs defaultValue="appointments" 
         ...
         >
            <TabsList className="md:col-span-1 bg-black/20 border h-14 md:h-40 flex sm:flex-row md:flex-col w-full p-2 md:p-1 rounded-md md:space-y-2 sm:space-x-2 md:space-x-0">
                <TabsTrigger 
                value="appointments"
                 className="flex-1 md:flex md:items-center md:justify-start md:px-4 md:py-3 w-full"
                >
                    <Calendar className="h-4 w-4 mr-2 hidden md:inline  text-cyan-400" />
                    <span>Appointment</span>
                    </TabsTrigger>
                <TabsTrigger 
                value="availability" 
                className="flex-1 md:flex md:items-center md:justify-start md:px-4 md:py-3 w-full">
                    <Clock className="h-4 w-4 mr-2 hidden md:inline  text-cyan-400" />
